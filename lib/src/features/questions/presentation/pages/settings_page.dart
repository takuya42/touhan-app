      ...
      body: ListView(
        ...
        children: [
          ...
          ListTile(
            ...
            title: const Text('AIチャット学習（準備中）'),
            subtitle: const Text('OpenAI API連携しやすい土台を用意済み'),
            onTap: () => Navigator.of(context).push(MaterialPageRoute<void>(builder: (_) => const _AiChatBasePage())),
          ),
          const Divider(),
...
class _AiChatBasePage extends StatelessWidget {
  const _AiChatBasePage();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: const Center(
        child: Text('将来的にOpenAI APIと接続するチャットUI土台です。'),
      ),
    );
  }
}
